...
export async function ERC20(token: any, init = Native): Promise<IERC20> {
    ...
    token = isNative
        ...
        ? await ethers.getContractAtFromArtifact(
              JSON.parse(require("fs").readFileSync(require("path").resolve(__dirname, "../artifacts/ERC20.sol/ERC20.json"))),
              token
          )
        : token;
    ...
    const address: AddressString = isNative
        ? init.address
    ...
    const module = (token: any, user?: IUser | AccountLike): IERC20Module => {
        ...
        const transfer = async (to: AccountLike | AddressString, amount: number | string): Promise<boolean | any> => {
            return isNative
                ? await ((user?.signer ? user?.signer : signers[0]) as Signer).sendTransaction({ to: a(to) as AddressLike, value: n(amount) })
                : await token.transfer(a(to), n(amount, decimals));
        };
        ...
        const transferFrom = async (from: IUser | AccountLike | AddressString, to: AccountLike | AddressString, amount: number): Promise<boolean | void> => {
            ...
            return isNative
                ? await signer.sendTransaction({ to: a(to) as AddressLike, value: n(amount) })
                : await (signer ? token.connect(signer) : token).transferFrom(a(from), a(to), n(amount, decimals));
        };
        ...
        const approve = async (spender: AccountLike | AddressString, amount: number | string | "max"): Promise<boolean | void> => {
            ...
            return isNative
                ? true
                : await (signer ? token.connect(signer) : token).approve(a(spender), amount === "max" ? ethers.MaxUint256 : n(amount, decimals));
        };
